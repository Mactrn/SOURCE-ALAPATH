...
async def _(zed):
    await edit_or_reply (zed, "𓆰 𝙎𝙊𝙐𝙍𝘾𝞝 𝙕𝞝𝘿 - 𝘿𝙀𝙇𝙀𝙏𝙀 🗑♻️𓆪\n𓍹━─━─━─━─𝙕𝞝𝘿─━─━─━─━𓍻\n\n ✵│رابـط الحـذف ↬ https://telegram.org/deactivate \n\n\n ✵│بـوت الحـذف  ↬ @LC6BOT ")
...
ZelzalTHS_cmd = (
    "𓆩 𝗦𝗼𝘂𝗿𝗰𝗲 𝗭𝗘𝗗𝗧𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش 𓆪\n\n"
...
async def zed(mention):
    ...
    await edit_or_reply(mention, f"✾╎نـسبـة حبكـم انـت و  {zedth} هـي {zedt} 😻♥️")
...
async def zed(mention):
    ...
    await edit_or_reply(mention, f"✾╎نسبـة الانوثه لـ  {zedth} هـي {zedt} 🤰")
...
async def permalink(mention):
    ...
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(mention, f"✾╎نسبـة الغبـاء لـ  {zedth} هـي {zedt} 😂💔")
...
async def permalink(mention):
    ...
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(mention, f"✾╎نسبـة الانحـراف لـ  {zedth} هـي {zedt} 🥵🖤")
...
async def permalink(mention):
    ...
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(mention, f"✾╎نسبـة المثليـه لـ  {zedth} هـي {zedt} 🤡 🏳️‍🌈.")
...
async def permalink(mention):
    ...
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(mention, f"✾╎نسبـة النجـاح لـ {zedth} هـي {zedt} 🤓.") 
...
async def permalink(mention):
    ...
    zedth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    zedt = random.choice(zid)
    await edit_or_reply(mention, f"✾╎نسبـة الكراهيـة لـ {zedth} هـي {zedt} 🤮.")
...
async def zed(mention):
    ...
    my_first = me.first_name
    my_mention = f"{me.first_name}"
    await edit_or_reply(mention, f"🚺 ╎ المستخـدم ه ⪼ •  {zedth2} \n☑️ ╎ تم رفعـها مـزة الكروب .. بنجـاح 🥳💃. \n🤵‍♂️ ╎ بواسطـه  : {my_mention} ")
